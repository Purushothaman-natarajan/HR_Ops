...
class LinUCBAgent:
    ...
    def select_action(self, context: dict) -> str:
        ...
        logger.debug("RL selected action=%s p_values=%s", selected, [round(p, 4) for p in p_values])
        return selected
    ...
    def _build_context_vector(self, context: dict) -> np.ndarray:
        vec = np.zeros(self.d)
        classification_map = {"policy": 0, "action": 1, "anomaly": 2, "compliance": 3}
    ...
    def save(self, path: Path | None = None):
        ...
        with open(p, "wb") as f:
            pickle.dump({"A": self.A, "b": self.b, "alpha": self.alpha, "gamma": self.gamma}, f)
