    ...
    if trigger in (TriggerType.REACTIVE, TriggerType.SYSTEM):
        ...
        if cached_agent:
            ...
            prompt = (
                ...
                f"Options:\n"
                f"- policy: questions about general HR policies, rules, benefits, guidelines, or documentation.\n"
                f"- action: queries seeking to retrieve, count, search, or modify employee details, databases, or records (e.g. employee lookup).\n"
                f"- hybrid: queries requiring BOTH database execution/lookup (action) AND policy explanation (policy). This includes checking if a specific employee request or data action is compliant with policies.\n"
                f"- anomaly: tasks investigating data discrepancies, outliers, errors, or anomalies, or instructions to run scans/checks.\n"
                f"- compliance: tasks checking general compliance audits, reports, status frameworks, or compliance officer escalations.\n\n"
                f"Examples:\n"
                f"- 'What is the maternity leave policy?' -> policy\n"
                f"- 'Find all engineering department staff' -> action\n"
                f"- 'Is sharing EMP0002 HR records with an external vendor compliant?' -> hybrid\n"
                f"- 'Run background anomaly check' -> anomaly\n"
                f"- 'Get compliance audit summary' -> compliance\n\n"
                f"Query: {state.query}\n"
                f"Reply with exactly one word from the options (policy, action, hybrid, anomaly, compliance)."
            )
            activities.append(Activity(
